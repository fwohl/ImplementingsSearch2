  ...
  //  }
	...
	for (int iii=0; iii<tree.size();iii++){
		...
		if (A.geterrors()==0){
			...
			//std::cout<<"legth results"<<results.size();
			int starti=0;
			int endi=0;
			for (auto result : results) {
				...
				if(foundpositions.size()!=0)
					std::cout<<"already found"<<foundpositions[0]<<std::endl;
				std::cout<<"new:"<<result.reference_begin_position()<<std::endl;
	...
	int newerrorcount=0;
	std::cout<<"size11"<<foundpositions.size()<<std::endl;
	...
	std::cout<<"sizefinh"<<finalhits.size();
	std::cout<<"finalhits:";
	for (int i=0;i<finalhits.size();i++) {
		std::cout<<finalhits[i];
	}
